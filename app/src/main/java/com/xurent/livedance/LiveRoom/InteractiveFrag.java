...
public class InteractiveFrag extends DialogFragment {
    ...
    private LiveFrag liveFrag;

    public InteractiveFrag(LiveFrag liveFrag) {
        this.liveFrag = liveFrag;
    }
        ...
        vp_interactive.setAdapter(new FragmentPagerAdapter(getChildFragmentManager()) {
            ...
            public Fragment getItem(int position) {
                if (position == 0) {
                    ...
                    layerFrag = new LayerFrag();
                    layerFrag.setOnOpenLiveListner(liveFrag);
                    return layerFrag; // 返回交互界面的frag
                } else { // 设置默认
